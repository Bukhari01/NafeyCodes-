...
const User = ({ item }) => {
  ...
  useEffect(() => {
    const fetchFriendRequests = async () => {
      try {
        const response = await fetch(`${BASE_URL}/friend-requests/sent/${userId}`);
    ...
    const fetchUserFriends = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log("error retrieving user freinds", response.status)
        }
      } catch (error) {
  ...
  const sendFriendRequest = async (currentUserId, selectedUserId) => {
    try {
      ...
      console.log("big error message", error);
    }
  };
